...
And, according to the premises, $0 < P( a = \mbox{T} ), P( b = \mbox{T} ) < 1$. So each term of the above equation must be positive, and our conclusion, $P( a = \mbox{F}\mid b = \mbox{F}) > P( b = \mbox{F})$, follows from the fact that a proper fraction of a positive number is less than that number.
